---
title: "individual_mediation_analysis"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{individual_mediation_analysis}
...
library(igraph)
```
# A mediation model with 10 continuous mediators,binary treatment
# and outcome
```{r}
...
a1=hcube(rep(2,10), scale = 1)
us=ifelse(a1==1,0, 1)
effect2=matrix(nrow = 2^10,ncol = 3)
s=c(1,2,5)
for (j in 1:3) {
  cat(j)
for (m in 1:2^10) {
  if (mod(m,20)==0){ cat("_",m, "_")}
  effect2[m,j]=beffect(
    index=s[j],
    u=us[m,],
