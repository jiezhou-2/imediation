...
(1) index specify the mediator of interest. The p mediators are indexed respectively by 1,2,...p.


(2) Vector u specify the state of treatment and other mediators, where 0 stands for no treatment, 1 treatment.
...
(4) data is a n by (p+q+2) matrix where the first column is treatment, the last column is the outcome, the 2 to (p+1) columns represents the mediators, the (p+2) to (p+q+1) columns represent the confounders.    
...
(6) type has two options, one is "continuous", which means outcome will by modeled by normal regression,  the other is "binomial", which means outcome will by modeled by logistic regression. In formar case, mediation effect is measured by the difference. In later case, mediation effect is measured by odds ratio. 
